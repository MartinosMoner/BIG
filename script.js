...
document.getElementById('installmentForm').addEventListener('submit', function (event) {
  ...
  const monthlyInstallment = totalAmount / totalMonths;
  ...
  document.getElementById('monthlyInstallment').textContent = `القسط الشهري: ${monthlyInstallment.toFixed(2)} وحدة نقدية`;
});
